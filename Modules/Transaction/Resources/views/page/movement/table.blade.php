    ...
    <tbody class="markup">
        ...
        <tr>
            <td data-title="ID Product">
                ...
                @else
                <a id="delete" value="{{ $item->movement_detail_item_product_id }}"
                    href="{{ route(config('module').'_delete', ['code' => $item->movement_detail_id, 'detail' => $item->movement_detail_item_product_id ]) }}"
                    class="btn btn-danger btn-xs btn-block delete-{{ $item->movement_detail_item_product_id }}">
                    {{ $item->movement_detail_item_product_id }}
                </a>
                @endif
                <input type="hidden" value="{{ $item['temp_id'] ?? $item->movement_detail_item_product_id }}" name="temp_id[]">
                <input type="hidden" value="{{ $item['temp_id'] ?? $item->movement_detail_item_product_id }}"
            ...
            <td data-title="Qty" class="text-right col-lg-1">
                <input type="text" tabindex="{{ $loop->iteration }}1" name="detail[{{ $loop->index }}][temp_qty]"
                    ...
                    value="{{ $item['temp_qty'] ?? $item->movement_detail_qty }}">

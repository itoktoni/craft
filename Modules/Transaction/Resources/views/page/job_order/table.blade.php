<table id="transaction" class="table table-no-more table-bordered table-striped">
    <thead>
        <tr>
            ...
            <th class="text-left col-md-4">Service Name</th>
    ...
    <tbody class="markup">
        ...
        <tr>
            <td data-title="ID Service">
                @if(old('detail'))
                <button id="delete" value="{{ $item['temp_id'] }}" type="button" class="btn btn-danger btn-xs btn-block">Delete {{ $item['temp_id'] }}</button>
                @else
                <a id="delete" value="{{ $item->mask_product_id }}" href="{{ route(config('module').'_delete', ['code' => $item->jo_detail_id, 'detail' => $item->mask_product_id ]) }}" class="btn btn-danger btn-xs btn-block delete-{{ $item->mask_product_id }}">
    ...
    <tbody>
        <tr>
            ...
            <td data-title="" class="text-right">
                {!! Form::text('jo_sum_value', null, ['id' => 'before_discount',
        ...
        <tr style="margin-bottom: 20px;">
            ...
            <td data-title="Description" class="text-left col-md-4">
                {!! Form::textarea('jo_discount_name', null, ['id' => 'grand_discount_description', 'class' =>
            ...
            <td data-title="Value" class="text-right col-md-1">
                {!! Form::text('jo_discount_value', null, ['class' => 'form-control text-right number', 'id' =>
            ...
            <td data-title="Discount" class="text-right col-md-1">
                {!! Form::text('jo_sum_discount', null, ['id' => 'grand_discount_price',
                'readonly', 'class' => 'number form-control text-right', 'tabindex' => 502]) !!}
            </td>
            <td data-title="Total" class="text-right col-md-1">
                {!! Form::text('jo_sum_total', null, ['id' => 'grand_discount_total',
                'readonly', 'class' => 'number form-control text-right']) !!}
            </td>
        </tr>
